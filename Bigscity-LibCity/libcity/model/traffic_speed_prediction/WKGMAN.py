...
class WKGMAN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.SE = torch.eye(self.num_nodes)
        # init logger
        ...
        self.add_day_in_week = self.data_feature.get('add_day_in_week', True)
        self.SE_fc = FC(input_dims=self.num_nodes, units=[self.D, self.D],
                        activations=[nn.ReLU, None], bn=self.bn, bn_decay=self.bn_decay, device=self.device)
        self.TE_fc = FC(input_dims=7 + self.T if self.add_day_in_week else self.T, units=[self.D, self.D],
                        activations=[nn.ReLU, None], bn=self.bn, bn_decay=self.bn_decay, device=self.device)
        self.ZE_fc = FC(input_dims=64, units=[self.D, self.D],
                        activations=[nn.ReLU, None], bn=self.bn, bn_decay=self.bn_decay, device=self.device)
        self.STE_fc = FC(input_dims=64, units=[self.D, self.D],
                        activations=[nn.ReLU, None], bn=self.bn, bn_decay=self.bn_decay, device=self.device)
