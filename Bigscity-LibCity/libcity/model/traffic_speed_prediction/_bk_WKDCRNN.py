...
class STEmbedding(nn.Module):
    def __init__(self, T, D, bn, bn_decay, add_day_in_week, device):
        ...
        self.bn_decay = bn_decay
        self.device = device
        self.SE_fc = GFC(input_dims=self.D, units=[self.D, self.D], activations=[nn.ReLU, None],
                        bn=self.bn, bn_decay=self.bn_decay, device=self.device)
        self.TE_fc = GFC(input_dims=7 + self.T if add_day_in_week else self.T, units=[self.D, self.D],
...
class SZEmbedding(nn.Module):
    def __init__(self, T, D, bn, bn_decay, add_day_in_week, device):
        super(SZEmbedding, self).__init__()
        self.T = T
        self.D = D
        self.bn = bn
        self.bn_decay = bn_decay
    ...
    def forward(self, SE, ZE):
        '''
        spatio-temporal embedding
        SE:     (num_nodes, D)
        ZE:     (batch_size, input_length+output_length, Z)
        retrun: (batch_size, input_length+output_length, num_nodes, D)
        '''
        SE = SE.unsqueeze(0).unsqueeze(0)
        SE = self.SE_fc(SE)
        return torch.add(SE, ZE)
...
class WKDCRNN(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        ...
        self.input_fc = GFC(input_dims=1, units=[self.D, self.D], activations=[nn.ReLU, None],
                           bn=self.bn, bn_decay=self.bn_decay, device=self.device)
        self.SE = nn.Parameter(torch.randn(self.num_nodes, self.D), requires_grad=True)
    ...
    def forward(self, batch, batches_seen=None):
        ...
        # x = x_all[:, :, :, 0:1]  # (batch_size, input_length, num_nodes, output_dim)
        # y = y_all[:, :, :, 0:1]  # (batch_size, input_length, num_nodes, output_dim)
        ...
        #     SE = SE.tile([batch_size, self.input_window+self.output_window, 1, 1])
        #     ZE = ZE.unsqueeze(2).to(device=self.device).to(device=self.device) 
        #     print('SE.shape, ZE.shape', SE.shape, ZE.shape)
        #     ste = torch.add(SE, ZE)
        # else: 
        ste = self.st_embedding(SE, TE)

        ste = ste.permute(1, 0, 2, 3)  # (total_window, batch_size, num_nodes, D)
        ste = ste.view(-1, batch_size, num_nodes * self.D).to(self.device)
        ste_p = ste[:self.input_window, ...]  # (total_window, batch_size, num_nodes * D)
        ste_q = ste[self.input_window:, ...]  # (total_window, batch_size, num_nodes * D)
