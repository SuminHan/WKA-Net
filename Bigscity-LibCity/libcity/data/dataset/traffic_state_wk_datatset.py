...
class TrafficStateWKDataset(AbstractDataset):
    """
    交通状态预测数据集的基类。
    ...
    def _generate_data(self):
        ...
        z = np.load(self.data_path + self.dataset + '_LIVEPOP.npy')
        # z: (num_samples, input_length+outpu_length, zemb_dim)

        # if self.config.get('model') in ['DCRNN', 'GTS', 'STGCN', 'STTN', 'GMAN', 'STAGGCN']:
        #     z = np.expand_dims(z, 2)
        #     # z: (num_samples, input_length+outpu_length, 1, zemb_dim)
        #     z = np.tile(z, [1, 1, x.shape[2], 1])

        #     x = np.concatenate((x[..., :1], z[:, :self.input_window, ...], x[..., 1:]), -1)
        #     y = np.concatenate((y[..., :1], z[:, self.input_window:, ...], y[..., 1:]), -1)
